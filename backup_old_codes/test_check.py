...
def predict_bcg_with_probabilities(image, model, feature_scaler=None, 
    ...
    if use_desprior_candidates:
        ...
        try:
            ...
            if len(file_candidates) == 0:
                ...
                print(f"  Candidate specific features shape: {candidate_specific_features.shape}")
                print(f"  Combined features shape: {features.shape}")
                if len(features) > 1:
                    # Save features to file for comparison
                    np.save('/tmp/test_check_features_cand1.npy', features[1, :])
                    print(f"  Candidate 1 features saved to /tmp/test_check_features_cand1.npy")
    ...
    if feature_scaler is not None:
        ...
        print(f"DEBUG SCALING:")
        print(f"  Raw features shape: {features.shape}")
        print(f"  Scaled features shape: {scaled_features.shape}")
        if len(scaled_features) > 1:
            # Save scaled features to file for comparison
            np.save('/tmp/test_check_scaled_cand1.npy', scaled_features[1, :])
            print(f"  Candidate 1 scaled features saved to /tmp/test_check_scaled_cand1.npy")
    else:
    ...
    with torch.no_grad():
        if hasattr(model, 'predict_with_uncertainty') and hasattr(model, 'temperature'):
            ...
            print(f"DEBUG: Features tensor shape: {features_tensor.shape}")
            print(f"DEBUG: First candidate raw logit: {raw_logits[0].item():.6f}")
            if len(raw_logits) > 1:
                print(f"DEBUG: Second candidate raw logit: {raw_logits[1].item():.6f}")
            ...
            print(f"DEBUG: Probabilities: {probabilities[:min(5, len(probabilities))]}")
...
def evaluate_enhanced_model(model, scaler, test_dataset, candidate_params, 
    ...
    for i in range(len(test_dataset)):
        ...
        print(f"DEBUG IMAGE LOADING:")
        print(f"  Filename: {filename}")
        print(f"  Image dtype: {image.dtype}")
        print(f"  Image shape: {image.shape}")
        print(f"  Image range: [{np.min(image):.1f}, {np.max(image):.1f}]")
