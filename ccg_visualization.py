...
1. Physical images with members showing p_{CCG} values
2. Diagnostic plots comparing p_{CCG} vs bar_p
3. Member distribution analysis around candidates
"""
...
# ============================================================================
# Physical Images with Members Visualization
# ============================================================================
def plot_cluster_with_members_pccg(cluster_name, image_path, candidates_pixel,
    ...
    # Plot cluster members with pmem coloring
    if members_df is not None and len(members_df) > 0:
        ...
        in_bounds = (member_x >= 0) & (member_x <= 512) & (member_y >= 0) & (member_y <= 512)
        member_x = member_x[in_bounds]
        member_y = member_y[in_bounds]
        members_in_bounds = members_df[in_bounds]

        # Plot members colored by pmem
        if 'pmem' in members_in_bounds.columns:
            pmem_values = members_in_bounds['pmem'].values
            norm = Normalize(vmin=0, vmax=1)
            cmap = plt.cm.viridis
            sm = ScalarMappable(norm=norm, cmap=cmap)

            for i in range(len(member_x)):
                edge_color = cmap(norm(pmem_values[i]))
                ax.scatter(member_x[i], member_y[i], marker='o', s=150,
            ...
            ax.scatter(member_x, member_y, marker='o', s=150,
                      facecolors='none', edgecolors='green',
                      linewidths=1.5, alpha=0.6, zorder=3)

    # Plot radius circles around candidates
    if redshift > 0 and not np.isnan(redshift):
        ...
        for i, (x, y) in enumerate(candidates_pixel):
            circle = plt.Circle((x, y), radius_pixels, fill=False,
                               color='gray', linestyle='--', alpha=0.5, linewidth=1)
            ax.add_patch(circle)

    # Plot candidates with colors based on rank
    colors = ["#FF0000", "#ff7b00", "#ff9900", "#ffe100", "#b7ff00",
             "#90ee90", "#87ceeb", "#dda0dd", "#f0e68c", "#ffa07a"]

    legend_elements = []

    # Sort candidates by bar_p
    ...
    for rank, idx in enumerate(sorted_indices):
        ...
        # Plot candidate
        ax.scatter(x, y, marker='o', s=800, facecolors='none',
                  edgecolors=color, linewidths=3, alpha=0.9, zorder=10)

        # Add label with both probabilities
        if not np.isnan(p_ccg):
            label_text = f'$\\bar{{p}}$:{bar_p:.2f}\n$p_{{CCG}}$:{p_ccg:.2f}\n$n_{{mem}}$:{int(n_mem)}'
        else:
            label_text = f'$\\bar{{p}}$:{bar_p:.2f}'

        ax.text(x + 12, y - 12, label_text, fontsize=10, color='black',
    ...
    if target_coords is not None:
        ...
        ax.scatter(tx, ty, marker='o', s=950, facecolors='none',
                  edgecolors="#59F5ED", linewidths=3, alpha=1.0,
    ...
    legend_elements.append(plt.Line2D([0], [0], marker='o', color='w',
                                     ...
                                     label=f'Members (N={len(members_df) if members_df is not None else 0})'))
    ...
    def format_relative_coordinate(offset_arcmin):
        ...
        arcsec = (abs_arcmin - arcmin_int) * 60
    ...
    ax.set_ylabel("Relative Position", fontsize=14)
...
def plot_pccg_vs_barp_diagnostic(p_ccg_results_df, output_dir, dataset_type='3p8arcmin'):
    ...
    ax = axes[1, 0]
    if 'n_members' in valid_df.columns:
        ax.scatter(valid_df['n_members'], valid_df['p_ccg'],
                  c=valid_df['bar_p'], cmap='RdYlGn',
                  alpha=0.6, s=50, edgecolors='black', linewidths=0.5)
        plt.colorbar(ax.collections[0], ax=ax, label='$\\bar{p}$')
    ...
    bars1 = ax.bar(x - width/2, mean_barp_by_z, width, label='$\\bar{p}$',
                  color='steelblue', edgecolor='black')
    bars2 = ax.bar(x + width/2, mean_pccg_by_z, width, label='$p_{CCG}$',
...
def generate_physical_images_with_members(p_ccg_detailed_results, image_dir,
    ...
    n_generated = 0
    for result in p_ccg_detailed_results:
        if max_images is not None and n_generated >= max_images:
            break
