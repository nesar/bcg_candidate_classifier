...
\begin{itemize}
\item \textbf{Basic color statistics}: 9 features (RGB means, standard deviations, and relative intensities)
\item \textbf{Color ratio features}: 7 features (approximating photometric indices like r-g, r-i colors)
\item \textbf{Spatial color variation}: 3 features (measuring spatial color uniformity)
\item \textbf{Color gradient features}: 8 features (including cross-channel gradient correlations)
\item \textbf{Convolution-based features}: 27 features (3 kernels × 3 channels × 3 response statistics)
\end{itemize}
