...
\subsection{Probabilistic Neural Network Training and Inference}

The framework implements a unified probabilistic approach that seamlessly integrates ranking-based training with uncertainty-aware inference. The complete pipeline transforms candidate features into calibrated probability estimates through the following methodology:

\subsubsection{Neural Network Architecture and Probabilistic Outputs}

The system employs a probabilistic neural network architecture (\texttt{BCGProbabilisticClassifier}) specifically designed for uncertainty-aware BCG identification:

\textbf{Network Architecture}: The probabilistic classifier implements a fully-connected multi-layer perceptron with hidden dimensions $[128, 64, 32]$ and ReLU activation functions, enhanced with uncertainty quantification capabilities. For typical feature dimensionality ($d = 40$), the complete network architecture is:

\begin{equation}
f_{\theta}(\mathbf{f}_i) = \mathbf{W}_4 \cdot \text{dropout}(\text{ReLU}(\mathbf{W}_3 \cdot \text{dropout}(\text{ReLU}(\mathbf{W}_2 \cdot \text{dropout}(\text{ReLU}(\mathbf{W}_1 \mathbf{f}_i + \mathbf{b}_1)) + \mathbf{b}_2)) + \mathbf{b}_3)) + \mathbf{b}_4
\end{equation}

where $\mathbf{W}_i$ and $\mathbf{b}_i$ represent the weights and biases of layer $i$, dropout with rate $\rho = 0.2$ is applied between all hidden layers, and the output layer produces raw logits for each candidate.

\textbf{Probabilistic Transformation}: The network outputs are converted to probabilities through sigmoid activation:
\begin{equation}
p_i = \sigma(f_{\theta}(\mathbf{f}_i))
\end{equation}
where $\sigma$ is the sigmoid function and $p_i$ represents the probability that candidate $i$ is the true BCG. The architecture includes a temperature parameter $\tau$ (initialized to 1.0) for potential future calibration, but the current implementation does not optimize this parameter during training.

\subsubsection{Training Procedure: From Ranking to Probabilities}

The training procedure establishes the connection between ranking-based supervision and probabilistic outputs:

\textbf{Step 1 - Candidate Detection and Feature Extraction}: For each training image, automatic algorithms identify multiple potential BCG locations $\{(x_1, y_1), (x_2, y_2), \ldots, (x_N, y_N)\}$ and extract feature vectors $\{\mathbf{f}_1, \mathbf{f}_2, \ldots, \mathbf{f}_N\}$ as described in Section 3.4.

\textbf{Step 2 - Ground Truth Assignment}: Among all candidates, we identify the target candidate closest to the known ground truth BCG location:
\begin{equation}
j^* = \argmin_j \|(x_j, y_j) - (x_{\text{true}}, y_{\text{true}})\|_2
\end{equation}
This candidate $j^*$ serves as the positive example during training.

\textbf{Step 3 - Probabilistic Ranking Loss}: The neural network processes all candidate features and produces raw logits. These logits are converted to probabilities and optimized using MarginRankingLoss to ensure proper ranking:
\begin{equation}
\mathcal{L}_{\text{ranking}} = \sum_{k \neq j^*} \max(0, \text{margin} - (s_{j^*} - s_k))
\end{equation}
where $s_i = f_{\theta}(\mathbf{f}_i)$ represents the raw network output (logits) for candidate $i$, and margin=0.5 enforces that the target candidate scores significantly higher than alternatives.


\subsubsection{Uncertainty Quantification Through Monte Carlo Dropout}

The framework implements epistemic uncertainty estimation through Monte Carlo dropout \citep{Gal2016MCDropout}, providing the probabilistic outputs $\bar{p}_i$ referenced throughout this work:

\textbf{Monte Carlo Sampling}: During inference, dropout remains active across $T$ forward passes with different dropout masks:
\begin{align}
...
where $\xi^{(t)}$ represents the dropout mask for forward pass $t$, $\bar{p}_i$ is the mean probability estimate (the probabilistic output used for BCG prediction), and $\text{Var}[p_i]$ quantifies epistemic uncertainty. The implementation uses $T = 10$ Monte Carlo samples, providing both point estimates and uncertainty bounds essential for scientific decision-making.

\textbf{Detection Threshold Classification}: The probabilistic outputs enable threshold-based classification for automated survey operations:
\begin{equation}
...
\end{cases}
\end{equation}
where $\theta_{\text{det}} = 0.5$ (configurable) serves as the detection threshold, transforming probabilistic outputs into binary classifications suitable for pipeline integration.

\textbf{Training Configuration}: The complete training procedure employs the following optimized hyperparameters:
\begin{itemize}
\item \textbf{Optimizer}: Adam with learning rate $\eta = 10^{-4}$, weight decay $\lambda = 10^{-4}$
...
\item \textbf{Training epochs}: 64 with early stopping based on validation performance
\item \textbf{Dropout rate}: $\rho = 0.2$ applied between all hidden layers for both regularization and uncertainty quantification
\item \textbf{Monte Carlo samples}: $T = 10$ dropout forward passes during inference
\end{itemize}
...
We evaluated the BCG probabilistic classifier on 385 galaxy clusters from the test dataset, achieving robust automated BCG identification performance with quantified uncertainties.

\subsection{Aggregate Results}

\textbf{Classification Performance}: The model achieves 80\% rank-1 accuracy, correctly identifying the BCG as the top-ranked candidate in 310 out of 385 test clusters. Including rank-2 predictions increases the success rate to 93\%, with rank-3 extending this to 96\%. This performance demonstrates effective BCG identification across diverse cluster environments.

\textbf{Spatial Accuracy}: 80\% of predictions achieve perfect spatial localization with zero pixel distance error between predicted and true BCG positions. For the remaining cases, the mean distance error is 23.6 pixels, corresponding to realistic uncertainties in BCG positioning within cluster cores given typical seeing conditions and galaxy extent.

\begin{figure}[htb]
\centering
\includegraphics[width=0.7\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/probability_analysis.png}
\caption{Probability distribution analysis showing detection thresholds and uncertainty quantification across the test dataset. The calibrated probabilities enable threshold-based classification essential for automated survey operations.}
\label{fig:prob_analysis}
\end{figure}

\textbf{Uncertainty Quantification}: Monte Carlo dropout provides calibrated uncertainty estimates with mean uncertainty of 0.072. The mean maximum probability across all clusters is 0.66, indicating confident predictions while maintaining realistic uncertainty bounds for scientific applications.

\subsection{Individual Results}

Representative examples demonstrate model behavior across different performance categories:

\begin{figure}[htb]
\centering
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/ProbabilisticTesting_prediction_sample_best_rank1_prediction_sample_1.png}
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/ProbabilisticTesting_prediction_sample_best_rank1_prediction_sample_2.png}
\caption{Successful rank-1 BCG identifications showing high-confidence predictions with clear morphological and photometric BCG characteristics. Probability values exceed 0.7 with low uncertainty estimates.}
\label{fig:rank1_examples}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/ProbabilisticTesting_prediction_sample_best_rank2_prediction_sample_1.png}
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/ProbabilisticTesting_prediction_sample_best_rank2_prediction_sample_2.png}
\caption{Rank-2 cases revealing multiple plausible BCG candidates within cluster cores. These scenarios often correspond to recent cluster mergers or complex central galaxy populations where astronomical ambiguity exists.}
\label{fig:rank2_examples}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/ProbabilisticTesting_prediction_sample_best_rest_prediction_sample_1.png}
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/evaluation_results/ProbabilisticTesting_prediction_sample_best_rest_prediction_sample_2.png}
\caption{Challenging failure cases (rank > 3) typically involving stellar contamination, edge effects, or genuine BCG identification ambiguity. The uncertainty quantification successfully flags these cases for manual inspection.}
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/feature_importance_analysis/plots/physical_interpretation/shap_physical_breakdown.png}
\includegraphics[width=0.48\textwidth]{/Users/nesar/Projects/HEP/IMGmarker/best_runs/runs_sep24/candidate_classifier_color_uq_run_20250926_004753/feature_importance_analysis/plots/physical_interpretation/shap_physical_importance.png}
\caption{SHAP analysis mapped to physical feature categories. Left: Breakdown of feature importance by physical domain. Right: Relative importance of astrophysical properties in BCG identification, demonstrating the model's reliance on observable galaxy characteristics.}
\label{fig:shap_physical}
\end{figure}

Physical interpretation of SHAP analysis reveals the astrophysical basis for automated BCG identification. Color information emerges as the dominant discriminative factor, reflecting the fundamental importance of stellar population properties and red-sequence membership in distinguishing cluster central galaxies from field contaminants. This validates the theoretical expectation that BCGs, as evolved massive ellipticals, exhibit characteristic spectral energy distributions distinct from younger stellar populations.

Morphological features provide the second-most important contribution, capturing the structural properties that distinguish BCGs through their central concentration, smooth light profiles, and environmental dominance within cluster cores. The significant role of contextual features confirms that BCG identification benefits from considering the broader cluster environment rather than isolated galaxy properties.

The physical feature breakdown demonstrates that successful automated BCG identification replicates the multi-wavelength, morphological approach traditionally employed by astronomers, with the uncertainty quantification providing calibrated confidence estimates essential for systematic survey applications.

\section{Conclusions and Future Directions}

We have presented a state-of-the-art machine learning framework for automated BCG identification that advances the field through key innovations: (1) comprehensive uncertainty quantification with epistemic uncertainty estimates implemented through Monte Carlo dropout, (2) explainable AI integration providing scientific interpretability via SHAP analysis, and (3) multi-modal feature extraction incorporating RGB color features for red-sequence detection, addressing fundamental limitations in traditional grayscale-based approaches.

\subsection{Key Contributions}

\textbf{Uncertainty-Aware Predictions}: Our implementation of Monte Carlo dropout \citep{Gal2016MCDropout} provides epistemic uncertainty estimates essential for scientific decision-making. The detection-threshold classification capability enables seamless integration with automated survey pipelines while maintaining quantified confidence levels.
