...
Recent work in BCG detection reflects this methodological evolution. \citet{Janulewicz2025} demonstrated that neural networks can achieve $R^2 \approx 0.94$ accuracy in automated BCG identification using purely data-driven coordinate regression approaches on simulated cluster data, while \citet{Ma2024sOPTICS} developed the sOPTICS algorithm combining modified density-based clustering with machine learning techniques for improved cluster detection. \citet{Manrique2022ICL} explored machine learning approaches for simultaneous BCG and intracluster light identification in simulated galaxy clusters, demonstrating the potential for multi-component analysis.
...
\end{equation}
where $m^*(z)$ represents the characteristic magnitude corresponding to the break in the galaxy luminosity function at redshift $z$ \citep{Rykoff2016DES}. This parameter provides powerful constraints on BCG candidacy based on established scaling relations, with typical BCG values $\Delta m^*_z < 1.0$ corresponding to stellar masses $M_* > 10^{11.5} M_{\odot}$. The framework supports configurable $\Delta m^*_z$ filtering ranges (typically [-4.0, 2.0]) for quality control and physical consistency validation.

The complete feature vector structure is as follows:
\begin{itemize}
\item \textbf{Core morphological features}: 30 dimensions
\item \textbf{RGB color features (PCA-reduced)}: 8 dimensions (when enabled)
\item \textbf{Auxiliary features}: 2 dimensions (redshift $z$, stellar mass indicator $\Delta m^*_z$; when available)
\item \textbf{Total dimensionality}: 40 dimensions (30 morphological + 8 color + 2 auxiliary when all features enabled)
\end{itemize}

The 30-dimensional core feature set includes: intensity statistics (5), central-peripheral analysis (3), gradient features (3), position features (3), shape/symmetry features (3), and multi-scale context features (13 from three radial scales). Features are standardized using \texttt{sklearn.preprocessing.StandardScaler} fitted on training data to ensure equal contribution across different scales and units, with $z$-score normalization: $f_{\text{norm}} = (f - \mu_{\text{train}})/\sigma_{\text{train}}$.
...
We implement SHapley Additive exPlanations (SHAP) analysis \citep{Lieu2025InterpretableAI} using the DeepExplainer method specifically designed for neural networks to quantify the contribution of each feature to individual predictions. For a given prediction $f(\mathbf{x})$, SHAP values $\phi_i$ satisfy:
\begin{equation}
...
Our framework incorporates comprehensive explainable AI capabilities through SHAP (SHapley Additive exPlanations) analysis \citep{Lieu2025InterpretableAI}, providing crucial scientific interpretability:
...
\textbf{RedMapper Supervision}: RedMapper BCG probabilities inform training through loss weighting while being explicitly excluded from inference features, ensuring scientific integrity and preventing data leakage \citep{Rykoff2014redMaPPer,Myles2025DESI}.
