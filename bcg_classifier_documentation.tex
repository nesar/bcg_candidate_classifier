...
\begin{itemize}
\item \textbf{Input Layer}: Variable feature dimension $d \in [32, 51]$ depending on enabled features (morphological: 18, color: 8 PCA components, luminosity: 6, auxiliary: 2)
...
\textbf{Parameter Count}: For typical feature dimensionality ($d = 32$), the network contains approximately 4,161 trainable parameters: $(32 \times 128 + 128) + (128 \times 64 + 64) + (64 \times 32 + 32) + (32 \times 1 + 1) = 4,161$ parameters, plus one additional temperature parameter $\tau$ for calibration.
